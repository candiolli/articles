...
public class Day1Trebuchet {
    ...
    static final Map<String, String> numberDictionary = new HashMap(
    ...
    private static Float extractNumbers(String input) {
        final String regex = "-?\\d+(?:\\d+)?|(?=(one|two|three|four|five|six|seven|eight|nine))";
        ...
        for (String c : groupsFound) {
            System.out.println("Day1Trebuchet.extractNumbers: " + c);
        }

        String firstNumber = getNumberFromText(groupsFound.get(0));
        String lastNumber = getNumberFromText(groupsFound.get(groupsFound.size() - 1));

        System.out.println(firstNumber + " - " + lastNumber);

        String join = String.join("",  firstNumber.substring(0,1),
                lastNumber.substring(lastNumber.length() == 1 ? 0 : lastNumber.length()-1));

        System.out.println("Input: " + input + ", extracted:"+ join);

        return Float.valueOf(join);
    }

    private static String getNumberFromText(String text) {
        Optional<Map.Entry<String, String>> first = numberDictionary.entrySet()
                .stream().filter(n -> n.getKey().equals(text))
                .findAny();
